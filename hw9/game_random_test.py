...
class TeekoPlayer:
    ...
    def succ_opp(self, state, opp):
        # generate successors for the opponent, return in format of [succ, [(target_x, target_y), (og_x, og_y)]
        # check if drop phase
        drop_phase = len([cell for row in state for cell in row if cell != ' ']) < 8
        successors = []
            ...
            for row in range(5):
                for col in range(5):
                    if state[row][col] == ' ':
                        ...
                        successor = [row[:] for row in state]
                        # place a piece on the empty space
                        successor[row][col] = self.opp
                        successors.append([successor, [(row, col)]])
        else:
            for row in range(5):
                for col in range(5):
                        ...
                        for row_move in range(-1,2):
                            for col_move in range(-1,2):
                                ...
                                if 0 <= row + row_move < 5 and 0 <= col + col_move < 5 and state[row + row_move][col + col_move] == ' ':
                                    ...
                                    successor = [row[:] for row in state]
                                    # place a piece on the empty space
                                    successor[row + row_move][col + col_move] = self.opp
                                    # remove the piece from its previous location
                                    successor[row][col] = ' '
                                    successors.append([successor, [(row + row_move, col + col_move), (row, col)]])
        return successors
